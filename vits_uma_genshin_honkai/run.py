...
class VITS(object):
    def __init__(self, device='cuda:0', 
        ...
        _, _, _, _ = utils.load_checkpoint(model_path, self.net_g_ms, None)

        # self.sid = self.speakers.index(speaker)
        self.lang = lang
    ...
    def __call__(self, text, output, sid):
        status, (_, data), msg = self.vits(text, self.lang, sid)
        logging.getLogger('PaddleSpeech').debug(f'{status}: {msg}')
        # data *= 32767 / max(0.01, np.max(np.abs(data))) * 0.6
        data = data / max(0.01, np.abs(data).max())
        data = data * 32767
        data = data.astype(np.int16)
    
        wavfile.write(output, self.hps_ms.data.sampling_rate, data)
